...
const projects = [
  {
    name: "dradix",
    desc: "Developer portfolio & analytics platform. Track your coding journey across all platforms in one place.",
  ...
  {
    name: "algo-vault",
    desc: "Curated collection of algorithms and data structures with step-by-step visualizations.",
  ...
  {
    name: "rustify",
    desc: "Rust learning platform with interactive challenges and progress tracking for beginners.",
    ...
    <div className="bg-[#161616] rounded-2xl p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {projects.map((p) => (
          <div
            key={p.name}
            className="bg-[#1c1c1c] rounded-xl overflow-hidden hover:bg-[#222] transition-colors group cursor-pointer"
          >
            <div
            ...
            >
              ...
              <div className="absolute top-3 right-3 flex items-center gap-1 text-[11px] text-[#888]">
                <StarIcon className="w-3 h-3" style={{ color: p.color }} />{" "}
                {p.stars}
              </div>
            </div>
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <p className="text-[15px] font-black text-white group-hover:text-white">
                  {p.name}
                </p>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {p.stack.map((tech) => (
                  <span
                    key={tech}
                    className="text-[10px] font-semibold text-[#888] bg-[#141414] rounded-md px-2 py-0.5"
                  >
                    {tech}
                  </span>
                ))}
