...
function Ring({
  ...
  return (
    <div className="flex flex-col items-center gap-2 cursor-default">
      ...
      <p className="text-[10px] font-semibold text-[#555] text-center leading-tight max-w-17">
        {label}
      </p>
    </div>
  );
  ...
  return (
    <div className="bg-[#161616] rounded-2xl p-5">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div>
          <p className="text-[11px] font-semibold text-[#555] mb-4">
            Progress Rings
          </p>
        ...
        <div className="space-y-4">
              ...
              {domains.map((s) => (
                <div key={s.label}>
                  <div className="flex justify-between text-[12px] mb-1.5">
                    ...
                    <span className="font-black" style={{ color: s.color }}>
                      {s.value}%
                    </span>
                  </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: "Overall Dev Score", value: "92", color: "#005c58" },
              { label: "Placement Readiness", value: "72%", color: "#3b82f6" },
            ].map((s) => (
              <div
                key={s.label}
                className="bg-[#1c1c1c] rounded-xl p-4 text-center hover:bg-[#222] transition-colors"
              >
