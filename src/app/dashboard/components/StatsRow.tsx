...
function CircularProgress({ value, size = 52, stroke = 4, color }: { value: number; size?: number; stroke?: number; color: string }) {
...
function Sparkline({ data, color }: { data: number[]; color: string }) {
  ...
  const w = 64, step = w / (data.length - 1);
  const path = norm.map((y, i) => `${i === 0 ? "M" : "L"} ${i * step} ${34 - y}`).join(" ");
  ...
  return (
    <section className="space-y-4">
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {cards.map((c) => (
          <div key={c.label} className="bg-[#161616] rounded-2xl p-5 hover:bg-[#1a1a1a] transition-colors cursor-default group">
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wider mb-1" style={{ color: c.color }}>{c.label}</p>
                <div className="flex items-end gap-1">
                  <span className="text-3xl font-black text-white tracking-tight">{c.value}</span>
                  {c.suffix && <span className="text-sm text-[#555] mb-1 ml-0.5">{c.suffix}</span>}
                </div>
                <div className="flex items-center gap-1 mt-1.5">
                  <ArrowUpIcon className="w-3 h-3" style={{ color: c.color }} />
                  <span className="text-[11px] font-medium" style={{ color: c.color }}>{c.change}</span>
                </div>
              </div>
              <div className="shrink-0">
                {c.type === "ring" && (
                  <div className="relative">
                    <CircularProgress value={c.ringValue!} size={52} stroke={4} color={c.color} />
                ...
                {c.type === "spark" && <Sparkline data={c.sparkData} color={c.color} />}
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex-1 h-0.5 rounded-full bg-[#222]">
                <div className="h-full rounded-full" style={{ backgroundColor: c.color, width: `${c.ringValue ?? 72}%` }} />
              </div>
      ...
      <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
        {miniStats.map((ms) => (
          <div key={ms.label} className="bg-[#161616] rounded-xl p-3 text-center hover:bg-[#1a1a1a] transition-colors cursor-default group">
            <p className="text-xl font-black text-white group-hover:scale-105 transition-transform inline-block">{ms.value}</p>
            <p className="text-[10px] font-semibold mt-0.5" style={{ color: ms.color }}>{ms.label}</p>
