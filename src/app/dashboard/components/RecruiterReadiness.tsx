    ...
    <div className="flex flex-col gap-4 h-full">
      <div className="relative bg-[#161616] border border-zinc-800/60 rounded-2xl p-5 overflow-hidden hover:border-zinc-700/80 transition-all flex-1">
        <div className="flex items-center gap-2 mb-5">
          <p className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">Recruiter Readiness</p>
        </div>
        ...
        <div className="flex items-center gap-6 mb-6">
          <div className="relative w-24 h-24 shrink-0">
            <svg width={96} height={96} className="rotate-[-90deg]">
              ...
              <circle
                cx={48} cy={48} r={38}
                stroke={score >= 80 ? "#a1a1aa" : score >= 60 ? "#71717a" : "#52525b"}
          ...
          <div>
            <p className="text-lg font-bold text-zinc-200 mb-1">
              {score >= 80 ? "Looking Great!" : score >= 60 ? "Almost There" : "Needs Work"}
            </p>
            <p className="text-[12px] text-zinc-500">Complete the missing items to stand out to recruiters.</p>
            {missing.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {missing.map((m) => (
                  <span key={m} className="text-[10px] text-zinc-400 bg-zinc-800 border border-zinc-700 rounded-full px-2 py-0.5">
                    {m}
                  </span>
                ))}
              </div>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {checklist.map((c) => (
            <div key={c.label} className="flex items-center gap-2">
              ...
              <p className={`text-[12px] ${c.done ? "text-zinc-400" : "text-zinc-600"}`}>{c.label}</p>
            </div>
          ))}
        </div>
      ...
      <div className="bg-[#161616] border border-zinc-800/60 rounded-2xl p-5 hover:border-zinc-700/80 transition-all">
        <p className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider mb-3">Public Profile</p>
        <div className="flex items-center gap-3 bg-zinc-900/60 border border-zinc-800/60 rounded-xl px-4 py-3 mb-3">
          <div className="w-1.5 h-1.5 rounded-full bg-zinc-400 animate-pulse" />
          <p className="text-[12px] text-zinc-300 font-mono">dradix.dev/yatharth</p>
        </div>
        <div className="grid grid-cols-2 gap-2">
          {["Open", "Share", "Copy Link", "Download CV"].map((action) => (
            <button key={action} className="py-2 text-[11px] text-zinc-400 bg-zinc-800/60 hover:bg-zinc-800 border border-zinc-700/60 rounded-lg transition-colors">
              {action}
            </button>
          ))}
        </div>
