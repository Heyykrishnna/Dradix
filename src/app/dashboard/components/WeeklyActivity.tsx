...
export default function WeeklyActivity() {
  ...
  const data = activeToggle === "Monthly" ? monthlyData : weeklyData;
  ...
  return (
    <div className="bg-[#161616] rounded-2xl p-5">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex bg-[#1c1c1c] rounded-xl p-0.5 gap-0.5">
          {toggles.map((t) => (
            <button
              key={t}
              onClick={() => setActiveToggle(t)}
              className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-all ${
                activeToggle === t ? "bg-[#00c9a7] text-black" : "text-[#555] hover:text-[#aaa]"
              }`}
            >
              {t}
            </button>
