...
export function GlassChatbot() {
  ...
  return (
    <>
      {/* Floating Trigger Launcher Button with Logo-DR.png */}
      {!isOpen && (
        <button
          onClick={() => setIsOpen(true)}
          className="fixed bottom-6 right-6 z-50 flex items-center justify-center p-2.5 bg-white/85 backdrop-blur-2xl backdrop-saturate-200 border border-white/90 shadow-[0_14px_45px_rgba(0,0,0,0.18),inset_0_1.5px_2px_rgba(255,255,255,1)] hover:bg-white hover:scale-110 active:scale-95 rounded-full transition-all duration-300 group cursor-pointer"
      ...
      {/* Main Glass Chatbot Modal Window */}
      {isOpen && (
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[calc(100vw-32px)] sm:w-110 max-h-[85vh] h-160 flex flex-col bg-white/75 backdrop-blur-2xl backdrop-saturate-200 border border-white/80 rounded-[32px] shadow-[0_30px_90px_rgba(0,0,0,0.2),inset_0_1.5px_2px_rgba(255,255,255,0.9)] overflow-hidden transition-all duration-300 animate-in fade-in zoom-in-95">
          ...
          <div className="relative z-10 flex items-center justify-between px-6 pt-5 pb-2">
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                {showOptionsMenu && (
                  <div className="absolute right-0 top-10 w-48 bg-white/95 backdrop-blur-xl border border-zinc-200/80 rounded-2xl shadow-xl p-1.5 z-30 space-y-1 text-xs">
                    <button
            ...
            {DEFAULT_PILLS.map((pill, idx) => {
              ...
              return (
                <button
                  ...
                  className="px-3.5 py-1.5 bg-white/80 hover:bg-white border border-white/90 shadow-[0_2px_8px_rgba(0,0,0,0.04)] rounded-full text-[11px] font-semibold text-zinc-800 flex items-center gap-1.5 shrink-0 transition-all hover:scale-105 active:scale-95 cursor-pointer"
                >
          ...
          {/* Expanded Extra Pills */}
          {showMorePills && (
            <div className="relative z-10 px-7 py-1.5 flex flex-wrap gap-2 animate-in fade-in duration-200">
              {EXTRA_PILLS.map((pill, idx) => (
                <button
                  key={idx}
                  onClick={() => handlePillClick(pill.prompt)}
                  className="px-3 py-1 bg-emerald-50/90 hover:bg-emerald-100 border border-emerald-200/80 rounded-full text-[10px] font-bold text-[#015451] transition-all hover:scale-105 cursor-pointer"
                >
                  {pill.label}
                </button>
              ))}
                      ...
                      >
                        <div className="flex items-start">
                          <div className="bg-white/85 backdrop-blur-md border border-white/90 p-3.5 rounded-2xl rounded-tl-xs shadow-xs text-xs text-zinc-600 font-medium flex items-center gap-2.5">
                            ...
                            <span>AI Assistant is analyzing your data...</span>
                          </div>
                        </div>
                      </MessageScrollerItem>
                    )}
          ...
          <div className="relative z-10 p-5 pt-2">
            <div className="bg-white/90 backdrop-blur-xl border border-white/95 shadow-[0_10px_30px_rgba(0,0,0,0.08),inset_0_1px_1px_rgba(255,255,255,1)] rounded-2xl p-3 space-y-2">
              ...
              {/* Context Attachment Options Drawer */}
              {showContextMenu && (
                <div className="p-2 bg-zinc-50 border border-zinc-200/80 rounded-xl space-y-1.5 text-[11px] animate-in fade-in duration-150">
              ...
              <div className="flex items-center justify-between pt-1">
                <button
                  ...
                  className="w-8 h-8 rounded-full bg-zinc-100 hover:bg-zinc-200 border border-zinc-200/70 flex items-center justify-center text-zinc-600 hover:text-zinc-950 transition-all cursor-pointer"
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-full bg-zinc-950 hover:bg-zinc-800 disabled:opacity-40 text-white flex items-center justify-center transition-all hover:scale-105 active:scale-95 shadow-md cursor-pointer"
                    title="Send Message"
                  >
